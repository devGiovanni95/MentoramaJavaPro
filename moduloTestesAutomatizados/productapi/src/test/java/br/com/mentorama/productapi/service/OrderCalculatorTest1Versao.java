...
class OrderCalculatorTest1Versao {
...
//
    @Test
    public void shouldCalculateOrderPrice(){
        final OrderCalculator1Versao orderCalculator = new OrderCalculator1Versao();
        final List<OrderItem> orderItems = anOrders();
        Double result = orderCalculator.calculatorOrder(orderItems);
        assertEquals(30.0,result);
...
podermos reutilizarmos ele na criacao de novos testes*/
    private List<OrderItem> anOrders() {
    ...
    public  void shouldCalculateTotalOfMultipleOrders(){
        final OrderCalculator1Versao orderCalculator = new OrderCalculator1Versao();
        final List<List<OrderItem>> orders = Arrays.asList(anOrders(),anOrders());
