...
//    }
    ...
    public CompletableFuture<List<ProfissionalSaudeEntity>> findByNumberMedicByDepartament() {
        CompletableFuture<List<ProfissionalSaudeEntity>> lista = profisssionalSaudeService.findByNumberMedicByDepartament();
        return this.profisssionalSaudeService.findByNumberMedicByDepartament();
    }

    @GetMapping
    public CompletableFuture<List<ProfissionalSaudeDTO>> findAll() {
        CompletableFuture<List<ProfissionalSaudeEntity>> lista = profisssionalSaudeService.findAll();
        CompletableFuture<List<ProfissionalSaudeDTO>> listDto = lista.stream().map(obj -> new ProfissionalSaudeDTO(obj)).collect(Collectors.toList());
